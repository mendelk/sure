---
pm-project: true
id: "p_alt_ingestion"
title: "03 Data Ingestion and Providers"
color: "#d18b47"
icon: "lucide-cloud-download"
taskIds: ["[[inventory-provider-connection-state-machines|Inventory provider connection state machines]]", "[[add-provider-management-api|Add provider management API]]", "[[build-provider-catalog-and-settings-ui|Build provider catalog and settings UI]]", "[[define-reusable-provider-connection-api-contract|Define reusable provider connection API contract]]", "[[implement-simplefin-connection|Implement SimpleFIN connection]]", "[[complete-import-session-api|Complete import session API]]", "[[build-resumable-import-wizard|Build resumable import wizard]]", "[[complete-family-export-lifecycle-api|Complete family export lifecycle API]]", "[[build-family-export-ui|Build family export UI]]", "[[verify-ingestion-and-simplefin-quality-gate|Verify ingestion and SimpleFIN quality gate]]"]
parent: "[[Projects/Sure Alt Frontend/Sure Alt Frontend]]"
customFields: []
teamMembers: []
savedViews: []
createdAt: "2026-09-07T01:09:35.000Z"
updatedAt: "2026-09-20T17:56:40.072Z"
config:
  defaultView: "table"
  autoSchedule: false
---

# lucide-cloud-download 03 Data Ingestion and Providers

Build shared provider infrastructure, deliver SimpleFIN, and complete import and export workflows without falling back to Rails HTML. Other concrete provider connections are deferred to the V2 Providers workstream.

## Tasks
- [ ] [[inventory-provider-connection-state-machines|Inventory provider connection state machines]]
- [ ] [[add-provider-management-api|Add provider management API]]
- [ ] [[build-provider-catalog-and-settings-ui|Build provider catalog and settings UI]]
- [ ] [[define-reusable-provider-connection-api-contract|Define reusable provider connection API contract]]
- [ ] [[implement-simplefin-connection|Implement SimpleFIN connection]]
- [ ] [[complete-import-session-api|Complete import session API]]
- [ ] [[build-resumable-import-wizard|Build resumable import wizard]]
- [ ] [[complete-family-export-lifecycle-api|Complete family export lifecycle API]]
- [ ] [[build-family-export-ui|Build family export UI]]
- [ ] [[verify-ingestion-and-simplefin-quality-gate|Verify ingestion and SimpleFIN quality gate]]
